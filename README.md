...
10.错误、调试和测试
	10-1.错误处理 
		...
		(2)调用栈：
			如果错误没有被捕获，他就会一直向上抛，最后被Python解释器捕获，打印一个错误信息，然后程序退出。
		(3)记录错误
		...
		(4)抛出错误
			...
			def foo(s):
				n = int(s)
				if n == 0
					raise FooError('invalib value:%s' %s)
				return 10/n
			foo('0')
	10-2.调试
		(1)调试
	...
	10-3.单元测试
		(1)单元测试
			又称"测试驱动开发"，单元测试是用来对一个模块、一个函数、或者一个类来进行正确性检验的测试工作。

11.IO编程 
 11-1文件读写 
 	(1)读文件 
 		open():f = open('Users/michael/test.txt','r')
